...
def val_one_epoch(model, Metric, val_loader, loss_function, device, hd95=False):
    """
    验证过程
    :param model: 模型
    :param metrics: 评估指标
    ...
    num_metrics = 5 if hd95 else 4
    Metrics_list = torch.zeros((num_metrics, 4), dtype=torch.float32, device=device)
    model.eval()
    val_et_loss = 0.0
    val_tc_loss = 0.0
    val_wt_loss = 0.0
    ...
    with torch.no_grad(): # 关闭梯度计算
        ...
        for data in val_loader:
            ...
            metrics = Metric.update(predicted_mask, mask, hd95=hd95)
    ...
    Metrics_list = Metrics_list.cpu().numpy()
