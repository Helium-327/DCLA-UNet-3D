...
'''
================================================
*      CREATE ON: 2025/03/30 11:27:25
*      AUTHOR: @Junyin Xiong
*      DESCRIPTION: 构建 Mega Res UNet 训练模型
*      VERSION: v1.0
*      FEATURES: 
*      STATE:     
*      CHANGE ON: 
...
from nnArchitecture.nets.mega_res_unets.v1 import (
    MegaResConv,
...
class MegaResUNetv1(nn.Module):
    def __init__(self, in_channels=4, out_channels=4, kernel_size=7, f_list=[32, 64, 128, 256], trilinear=True):
        ...
        self.bottleneck = MegaResConv(f_list[3], f_list[3], kernel_size=kernel_size)
        
        self.Up5 = UpSample(f_list[3], f_list[3], trilinear)
        self.UpConv5 = MegaResConv(f_list[3]*2, f_list[3]//2, kernel_size=kernel_size)
        
        self.Up4 = UpSample(f_list[2], f_list[2], trilinear)
        self.UpConv4 = MegaResConv(f_list[2]*2, f_list[2]//2, kernel_size=kernel_size)
        
        self.Up3 = UpSample(f_list[1], f_list[1], trilinear)
        self.UpConv3 = MegaResConv(f_list[1]*2, f_list[1]//2, kernel_size=kernel_size)
        
        self.Up2 = UpSample(f_list[0], f_list[0], trilinear)
        self.UpConv2 = MegaResConv(f_list[0]*2, f_list[0], kernel_size=kernel_size)
...
class MegaResUNetv1_DCLA(MegaResUNetv1):
    def __init__(self, 
                in_channels=4, 
                out_channels=4, 
        ...
        super(MegaResUNetv1_DCLA, self).__init__(
...
class MegaResUNetv1_DCLA_AG(MegaResUNetv1_DCLA):
    def __init__(self, 
                in_channels=4, 
                out_channels=4, 
                f_list=[32, 64, 128, 256], 
        ...
        super(MegaResUNetv1_DCLA_AG, self).__init__(
            in_channels=in_channels, 
            out_channels=out_channels, 
            f_list=f_list, 
            trilinear=trilinear)
...
class MegaResUNetv1_DCLA_MADG(MegaResUNetv1_DCLA_AG):
    def __init__(self, 
                in_channels=4, 
                out_channels=4, 
                f_list=[32, 64, 128, 256], 
        ...
        super(MegaResUNetv1_DCLA_MADG, self).__init__(
            in_channels=in_channels, 
            out_channels=out_channels, 
            f_list=f_list, 
            trilinear=trilinear)
...
if __name__ == "__main__":
    test_unet(model_class=MegaResUNetv1_DCLA_MADG, batch_size=1)   