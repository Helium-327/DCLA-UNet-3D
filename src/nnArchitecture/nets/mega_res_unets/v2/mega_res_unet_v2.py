...
'''
================================================
*      CREATE ON: 2025/05/05 12:08:54
*      AUTHOR: @Junyin Xiong
*      DESCRIPTION:  构建  Mega Res UNet v2
*      VERSION: v2.0
*      FEATURES: 
*      STATE:     
*      CHANGE ON: 
...
class MegaResUNetv2(nn.Module):
    def __init__(self, in_channels=4, out_channels=4, kernel_size=7, f_list=[32, 64, 128, 256], trilinear=True):
        super(MegaResUNetv2, self).__init__()
        self.MaxPool = nn.MaxPool3d(kernel_size=2, stride=2)
        self.Conv1 = MegaResConv(in_channels, f_list[0], kernel_size=kernel_size)
        self.Conv2 = MegaResConv(f_list[0], f_list[1], kernel_size=kernel_size)
        self.Conv3 = MegaResConv(f_list[1], f_list[2], kernel_size=kernel_size)
        self.Conv4 = MegaResConv(f_list[2], f_list[3], kernel_size=kernel_size)
        
        self.bottleneck = ResConv3D_S_BN(f_list[3], f_list[3])
        
        self.Up5 = UpSample(f_list[3], f_list[3], trilinear)
        self.UpConv5 = ResConv3D_S_BN(f_list[3]*2, f_list[3]//2)
        
        self.Up4 = UpSample(f_list[2], f_list[2], trilinear)
        self.UpConv4 = ResConv3D_S_BN(f_list[2]*2, f_list[2]//2)
        
        self.Up3 = UpSample(f_list[1], f_list[1], trilinear)
        self.UpConv3 = ResConv3D_S_BN(f_list[1]*2, f_list[1]//2)
        
        self.Up2 = UpSample(f_list[0], f_list[0], trilinear)
        self.UpConv2 = ResConv3D_S_BN(f_list[0]*2, f_list[0])
...
class MegaResUNetv2_DCLA(MegaResUNetv2):
    def __init__(self, 
                in_channels=4, 
                out_channels=4, 
...
class MegaResUNetv2_DCLA_AG(MegaResUNetv2_DCLA):
    def __init__(self, 
                in_channels=4, 
                out_channels=4, 
                f_list=[32, 64, 128, 256], 
        ...
        super(MegaResUNetv2_DCLA_AG, self).__init__(
            in_channels=in_channels, 
            out_channels=out_channels, 
            f_list=f_list, 
            trilinear=trilinear)
...
class MegaResUNetv2_DCLA_MADG(MegaResUNetv2_DCLA_AG):
    def __init__(self, 
                in_channels=4, 
                out_channels=4, 
                f_list=[32, 64, 128, 256], 
        ...
        super(MegaResUNetv2_DCLA_MADG, self).__init__(
            in_channels=in_channels, 
            out_channels=out_channels, 
            f_list=f_list, 
            trilinear=trilinear)
...
class MegaResUNetv2_LKDCLA(MegaResUNetv2_DCLA):
    def __init__(self, in_channels=4, out_channels=4, kernel_size=7, f_list=[32, 64, 128, 256], trilinear=True):
        super(MegaResUNetv2_LKDCLA, self).__init__(
            in_channels=in_channels, 
            out_channels=out_channels, 
            f_list=f_list, 
            kernel_size=kernel_size,
...
class MegaResUNetv2_SLKDCLA(MegaResUNetv2_LKDCLA):
    def __init__(self, in_channels=4, out_channels=4, kernel_size=11, f_list=[32, 64, 128, 256], trilinear=True):
        super(MegaResUNetv2_SLKDCLA, self).__init__(
            in_channels=in_channels, 
            out_channels=out_channels, 
            f_list=f_list, 
            kernel_size=kernel_size,
...
if __name__ == "__main__":
    test_unet(model_class=MegaResUNetv2_LKDCLA, batch_size=1)   