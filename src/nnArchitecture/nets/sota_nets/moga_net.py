...
class MogaNet(nn.Module):
    def __init__(
        ...
        self.out = nn.ModuleList(
            [Out(channels[depth - i - 1], out_channels) for i in range(depth)]
        )
        # self.out[-1] = MSCHeadv6(channels[0], out_channels, 7)
        # self.out[-1] = MSCHead_moe(channels[0], out_channels, 7)
        # self.out[-1]=MSCHead_nonMoe_skfv3_cbam(channels[0], out_channels, 7)
        # self.softmax = nn.Softmax(dim=1)
