...
from nnArchitecture.nets.dcla_nets.dcla_unet_250607.mm import *
...
class ResUNeXt_250607(nn.Module):
    __remark__ = """
    [Version]: 0604
    [Author]: Junyin Xiong
    [Features]
    ...
    """
    def __init__(self, in_channels=4, out_channels=4, kernel_size=3, f_list=[32, 64, 128, 256], trilinear=True):
        super(ResUNeXt_250607, self).__init__()
...
class ResUNeXt_DCLA_250607(ResUNeXt_250607):
    __remark__ = """
    [Version]: 0604
    [Author]: Junyin Xiong
    [Features]
    ...
    """
    def __init__(self, in_channels=4, out_channels=4, kernel_size=3, f_list=[32, 64, 128, 256], trilinear=True):
        super(ResUNeXt_DCLA_250607, self).__init__()
    ...
    def forward(self, x):
        ...
        x5 = self.dcla([x1, x2, x3, x4], x5)  if hasattr(self, "dcla") else x5
...
class DCLA_UNet_250607(nn.Module):
    __remark__ = """
    [Version]: 250607
    [Author]: Junyin Xiong
    [Features]
    ...
    def __init__(self, 
        ...
        super(DCLA_UNet_250607, self).__init__()
        self.MaxPool = nn.MaxPool3d(kernel_size=2, stride=2)
...
class DCLA_UNet_NoRes_250607(nn.Module):
    __remark__ = """
    [Version]: 250607
    [Author]: Junyin Xiong
    [Features]
    ...
    def __init__(self, 
        ...
        super(DCLA_UNet_NoRes_250607, self).__init__()
        self.MaxPool = nn.MaxPool3d(kernel_size=2, stride=2)
        self.Conv1 = self._make_encoder_layer(in_channels, f_list[0], kernel_size=kernel_size, se_ratio=se_ratio)
        self.Conv2 = self._make_encoder_layer(f_list[0], f_list[1], kernel_size=kernel_size, se_ratio=se_ratio)
        self.Conv3 = self._make_encoder_layer(f_list[1], f_list[2], kernel_size=kernel_size, se_ratio=se_ratio)
    ...
    def _make_decoder_layer(self, 
        ...
        return nn.Sequential(
            EfficientAttentionBlock(in_channels, ratio=16, spatial_kernal=7, parallel=True),
            MutilScaleFusionBlock(in_channels, out_channels, dilations=[1, 2, 3], norm_type=norm_type, act_type=act_type)
        )
...
class SLK_UNet_250607(DCLA_UNet_250607):
    ...
    def __init__(self,
        ...
        super(SLK_UNet_250607, self).__init__()
...
class SLK_SE_UNet_250607(DCLA_UNet_250607):
    __remark__ = """
    [Version]: 250607
    [Author]: Junyin Xiong
    [Features]
    ...
    def __init__(self,
        ...
        super(SLK_SE_UNet_250607, self).__init__()
        if hasattr(self, "dcla"):
            delattr(self, "dcla")
            
        self.Conv1 = nn.Sequential(
            SlimLargeKernelBlockv4(in_channels, f_list[0], kernel_size=kernel_size, norm_type=norm_type, act_type=act_type),
            SqueezeExcitation(f_list[0], reduction_ratio=16)
        )
        self.Conv2 = nn.Sequential(
            SlimLargeKernelBlockv4(f_list[0], f_list[1], kernel_size=kernel_size, norm_type=norm_type, act_type=act_type),
            SqueezeExcitation(f_list[1], reduction_ratio=16)
        )
        self.Conv3 = nn.Sequential(
            SlimLargeKernelBlockv4(f_list[1], f_list[2], kernel_size=kernel_size, norm_type=norm_type, act_type=act_type),
            SqueezeExcitation(f_list[2], reduction_ratio=16)
        )
        self.Conv4 = nn.Sequential(
            SlimLargeKernelBlockv4(f_list[2], f_list[3], kernel_size=kernel_size, norm_type=norm_type, act_type=act_type),
            SqueezeExcitation(f_list[3], reduction_ratio=16)
        )
        
        self.UpConv4 = nn.Sequential(
            SqueezeExcitation(f_list[3]*2, reduction_ratio=16),
            SlimLargeKernelBlockv4(f_list[3]*2, f_list[3]//2, kernel_size=kernel_size, norm_type=norm_type, act_type=act_type)
        )
        self.UpConv3 = nn.Sequential(
            SqueezeExcitation(f_list[2]*2, reduction_ratio=16),
            SlimLargeKernelBlockv4(f_list[2]*2, f_list[2]//2, kernel_size=kernel_size, norm_type=norm_type, act_type=act_type)
        )
        self.UpConv2 = nn.Sequential(
            SqueezeExcitation(f_list[1]*2, reduction_ratio=16),
            SlimLargeKernelBlockv4(f_list[1]*2, f_list[1]//2, kernel_size=kernel_size, norm_type=norm_type, act_type=act_type)
        )
        self.UpConv1 = nn.Sequential(
            SqueezeExcitation(f_list[0]*2, reduction_ratio=16),
            SlimLargeKernelBlockv4(f_list[0]*2, f_list[0], kernel_size=kernel_size, norm_type=norm_type, act_type=act_type)
        )
    def forward(self, x):
        return super().forward(x)
    
class MSF_UNet_250607(DCLA_UNet_250607):
    __remark__ = """
    [Version]: 250607
    [Author]: Junyin Xiong
    [Features]
    ...
    def __init__(self,
        ...
        super(MSF_UNet_250607, self).__init__()
        if hasattr(self, "dcla"):
            delattr(self, "dcla")
...
class SLK_MSF_UNet_250607(DCLA_UNet_250607):
    __remark__ = """
    [Version]: 250607
    [Author]: Junyin Xiong
    [Features]
    ...
    def __init__(self,
        ...
        super(SLK_MSF_UNet_250607, self).__init__()
        if hasattr(self, "dcla"):
            delattr(self, "dcla")
...
class SLK_MSF_DCLA_UNet_250607(DCLA_UNet_250607):
    __remark__ = """
    [Version]: 250607
    [Author]: Junyin Xiong
    [Features]
    ...
    def __init__(self,
        ...
        super(SLK_MSF_DCLA_UNet_250607, self).__init__()
...
class SLK_MSF_DCLA_k7_NoRes_UNet_250607(DCLA_UNet_NoRes_250607):
    __remark__ = """
    [Version]: 250609
    [Author]: Junyin Xiong
    [Features]
    ...
    def __init__(self,
        ...
        super(SLK_MSF_DCLA_k7_NoRes_UNet_250607, self).__init__()
            
        self.Conv1 = SlimLargeKernelBlockv4(in_channels, f_list[0], kernel_size=kernel_size, norm_type=norm_type, act_type=act_type)
        self.Conv2 = SlimLargeKernelBlockv4(f_list[0], f_list[1], kernel_size=kernel_size, norm_type=norm_type, act_type=act_type)
        self.Conv3 = SlimLargeKernelBlockv4(f_list[1], f_list[2], kernel_size=kernel_size, norm_type=norm_type, act_type=act_type)
        self.Conv4 = SlimLargeKernelBlockv4(f_list[2], f_list[3], kernel_size=kernel_size, norm_type=norm_type, act_type=act_type)
        self.dcla = DynamicCrossLevelAttention(ch_list=f_list, feats_size=[128, 64, 32, 16], min_size=8, squeeze_kernel=1, down_kernel=[7], fusion_kernel=7)
        self.UpConv4 = MutilScaleFusionBlock(f_list[3]*2, f_list[3]//2, dilations=[1, 2, 3], norm_type=norm_type, act_type=act_type)
        self.UpConv3 = MutilScaleFusionBlock(f_list[2]*2, f_list[2]//2, dilations=[1, 2, 3], norm_type=norm_type, act_type=act_type)
        self.UpConv2 = MutilScaleFusionBlock(f_list[1]*2, f_list[1]//2, dilations=[1, 2, 3], norm_type=norm_type, act_type=act_type)
        self.UpConv1 = MutilScaleFusionBlock(f_list[0]*2, f_list[0], dilations=[1, 2, 3], norm_type=norm_type, act_type=act_type)
...
class AxFB_UNet_250607(DCLA_UNet_250607):
    __remark__ = """
    [Version]: 250607
    [Author]: Junyin Xiong
    [Features]
    ...
    def __init__(self,
        ...
        super(AxFB_UNet_250607, self).__init__()
            
        self.Conv1 = AxioFusionBlock(in_channels, f_list[0], kernel_size=kernel_size, norm_type=norm_type, act_type=act_type)
        self.Conv2 = AxioFusionBlock(f_list[0], f_list[1], kernel_size=kernel_size, norm_type=norm_type, act_type=act_type)
        self.Conv3 = AxioFusionBlock(f_list[1], f_list[2], kernel_size=kernel_size, norm_type=norm_type, act_type=act_type)
        self.Conv4 = AxioFusionBlock(f_list[2], f_list[3], kernel_size=kernel_size, norm_type=norm_type, act_type=act_type)
...
class SLK_MSF_SE_UNet_250607(DCLA_UNet_250607):
    ...
    def __init__(self,
        ...
        super(SLK_MSF_SE_UNet_250607, self).__init__()
        if hasattr(self, "dcla"):
            delattr(self, "dcla")
        self.Conv1 = nn.Sequential(
            SlimLargeKernelBlockv4(in_channels, f_list[0], kernel_size=kernel_size, norm_type=norm_type, act_type=act_type),
...
class SLK_MSF_SEdown_UNet_250607(DCLA_UNet_250607):
    __remark__ = """
    [Version]: 250607
    [Author]: Junyin Xiong
    [Features]
    ...
    def __init__(self,
        ...
        super(SLK_MSF_SEdown_UNet_250607, self).__init__()
        if hasattr(self, "dcla"):
            delattr(self, "dcla")
        ...
        self.UpConv4 = nn.Sequential(
            SqueezeExcitation(f_list[3]*2, reduction_ratio=16),
            MutilScaleFusionBlock(f_list[3]*2, f_list[3]//2, dilations=[1, 2, 3], norm_type=norm_type, act_type=act_type)
        )
        self.UpConv3 = nn.Sequential(
            SqueezeExcitation(f_list[2]*2, reduction_ratio=16),
            MutilScaleFusionBlock(f_list[2]*2, f_list[2]//2, dilations=[1, 2, 3], norm_type=norm_type, act_type=act_type)
        )
        self.UpConv2 = nn.Sequential(
            SqueezeExcitation(f_list[1]*2, reduction_ratio=16),
            MutilScaleFusionBlock(f_list[1]*2, f_list[1]//2, dilations=[1, 2, 3], norm_type=norm_type, act_type=act_type)
        )
        self.UpConv1 = nn.Sequential(
            SqueezeExcitation(f_list[0]*2, reduction_ratio=16),
            MutilScaleFusionBlock(f_list[0]*2, f_list[0], dilations=[1, 2, 3], norm_type=norm_type, act_type=act_type)
        )
    def forward(self, x):
        return super().forward(x)
    
class SLK_MSF_SE_DCLA_UNet_250607(DCLA_UNet_250607):
    __remark__ = """
    [Version]: 250607
    [Author]: Junyin Xiong
    [Features]
    ...
    def __init__(self,
        ...
        super(SLK_MSF_SE_DCLA_UNet_250607, self).__init__()
