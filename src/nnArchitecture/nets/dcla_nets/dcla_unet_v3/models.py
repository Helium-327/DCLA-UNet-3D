...
#         self.dcla = DCLA(ch_list=f_list, feats_size=[128, 64, 32, 16], min_size=8, squeeze_kernel=1, down_kernel=[7], fusion_kernel=1, norm_type=norm_type, act_type=act_type)
#         self.Up4 = UpSample(f_list[3], f_list[3], trilinear)
#         self.Up3 = UpSample(f_list[2], f_list[2], trilinear)
#         self.Up2 = UpSample(f_list[1], f_list[1], trilinear)
#         self.Up1 = UpSample(f_list[0], f_list[0], trilinear)
...
class DCLA_UNet_v2_4(nn.Module):
    ...
    def __init__(self, 
        ...
        self.MaxPool = nn.MaxPool3d(kernel_size=2, stride=2)
        self.Conv1 = SLKv2(in_channels, f_list[0], kernel_size=kernel_size, norm_type=norm_type, act_type=act_type)
        self.dcla1 = DCLA(ch_list=[f_list[0]], feats_size=[128], min_size=64, squeeze_kernel=1, down_kernel=[7], fusion_kernel=1, norm_type=norm_type, act_type=act_type)
        self.Conv2 = SLKv2(f_list[0],f_list[1], kernel_size=kernel_size,norm_type=norm_type, act_type=act_type)
        self.dcla2 = DCLA(ch_list=[f_list[1]], feats_size=[64],  min_size=32, squeeze_kernel=1, down_kernel=[7], fusion_kernel=1, norm_type=norm_type, act_type=act_type)
        self.Conv3 = SLKv2(f_list[1], f_list[2], kernel_size=kernel_size, norm_type=norm_type, act_type=act_type)
        self.dcla3 = DCLA(ch_list=[f_list[2]], feats_size=[32], min_size=16, squeeze_kernel=1, down_kernel=[7], fusion_kernel=1, norm_type=norm_type, act_type=act_type)
        self.Conv4 = SLKv2(f_list[2], f_list[3], kernel_size=kernel_size, norm_type=norm_type, act_type=act_type)
        self.dcla4 = DCLA(ch_list=f_list, feats_size=[128, 64, 32, 16], min_size=8, squeeze_kernel=1, down_kernel=[7], fusion_kernel=1, norm_type=norm_type, act_type=act_type)
    ...
    def forward(self, x):
        ...
        x2 = self.Conv2(x2)      # [B, 64, D/2, H/2, W/2]
        x3 = self.MaxPool(x2)
        x3 = self.dcla2([x2], x3) if hasattr(self, 'dcla2') else x3
...
class ResUNetBaseline_S_SLKv2_MSF_v2(DCLA_UNet_v2_3):
    __remark__ = """
    [Version]: V2
    [Author]: Junyin Xiong
    [basline]: ResUNetBaseline_S
