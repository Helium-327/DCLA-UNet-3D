...
from nnArchitecture.nets.dcla_nets.dcla_unet_250602.modules import *
...
class DCLA_UNet_250602(nn.Module):
    __remark__ = """
    [Version]: 250602
    [Author]: Junyin Xiong
    [Features]
    ...
    def __init__(self, 
        ...
        super(DCLA_UNet_250602, self).__init__()
        self.MaxPool = nn.MaxPool3d(kernel_size=2, stride=2)
        self.Conv1 = MutilScaleFusionBlock(in_channels, f_list[0],  use_attn=False)
        self.Conv2 = MutilScaleFusionBlock(f_list[0], f_list[1],  use_attn=False)
        self.Conv3 = MutilScaleFusionBlock(f_list[1], f_list[2],  use_attn=False)
        self.Conv4 = MutilScaleFusionBlock(f_list[2], f_list[3],  use_attn=False)

        self.dcla = EnhancedCrossLevelAttention(ch_list=f_list, feats_size=[128, 64, 32, 16], min_size=8)
        ...
        self.UpConv4 = MutilScaleFusionBlock(in_channels=f_list[3]*2, out_channels=f_list[3]//2,  use_attn=False)
        self.UpConv3 = MutilScaleFusionBlock(in_channels=f_list[2]*2, out_channels=f_list[2]//2,  use_attn=False)
        self.UpConv2 = MutilScaleFusionBlock(in_channels=f_list[1]*2, out_channels=f_list[1]//2,  use_attn=False)
        self.UpConv1 = MutilScaleFusionBlock(in_channels=f_list[0]*2, out_channels=f_list[0],     use_attn=False)
...
class BaseLine_S_DCLA_250602(DCLA_UNet_250602):
    __remark__ = """
    [Version]: 250602
    [Author]: Junyin Xiong
    [basline]: ResUNetBaseline_S
    ...
    def __init__(self,
        ...
        super(BaseLine_S_DCLA_250602, self).__init__(
                in_channels=in_channels,
                out_channels=out_channels,
                f_list=f_list,
                trilinear=trilinear,
...
if __name__ == "__main__":
    test_unet(model_class=BaseLine_S_DCLA_250602, batch_size=1)   
    model = BaseLine_S_DCLA_250602(in_channels=4, out_channels=4)
