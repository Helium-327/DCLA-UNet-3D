...
from nnArchitecture.nets.dcla_nets.dev.baseline_dev import (
    ResUNetBaseline_S
)
...
class DCLA_UNet(nn.Module):
    ...
    def __init__(self, 
        ...
        self.bottle_neck = MutilScaleFusionBlock(in_channels=f_list[3], out_channels=f_list[3], fusion_kernel=7)
...
class ResUNetBaseline_S_DCLA_SLK(ResUNetBaseline_S_SLK):
    __remark__ = """
    [Version]: V1.8
    [Author]: Junyin Xiong
    • 总参数量: 2.765M
...
class ResUNetBaseline_S_DCLA_LiteMSF(ResUNetBaseline_S_LiteMSF):
    __remark__ = """
    [Version]: V1.8
    [Author]: Junyin Xiong
    • 总参数量: 3.223M
...
class ResUNetBaseline_S_SLK_LiteMSF(DCLA_UNet):
    __remark__ = """
    [Version]: V1.8
    [Author]: Junyin Xiong
    • 总参数量: 916.524K
...
class DynamicCrossLevelAttention(nn.Module): #MSFA
    def __init__(self, 
        ...
        # self.channels_attn = MultiLevelChannelAttention(ch_list=self.ch_list, ratio=4)
    ...
    def forward(self, encoder_feats, x):
        ...
        spatial_attn = torch.sigmoid(fused)
        
        # channel_attn = self.channels_attn(encoder_feats, x)
        out = spatial_attn * x  + x
        
        return out
    
class MultiLevelChannelAttention(nn.Module):
    def __init__(self, ch_list, ratio = 2):
        super().__init__()
        self.ch_list = ch_list
        self.ratio = ratio
        ...
        self.channnel_attn = nn.Sequential(
            nn.Conv3d(
                sum(self.ch_list), 
                sum(self.ch_list)//self.ratio, 
                kernel_size=1
            ),
            Swish(),
            nn.Conv3d(
    ...
    def forward(self, encoder_feats, x):
        ...
        feats = [self.gap(feat)+self.gmp(feat)  for feat in encoder_feats]
        feats = torch.cat(feats, dim=1)
        
        attn = self.channnel_attn(feats)
        return attn
