...
class DCLA_UNet(nn.Module):
    ...
    • 总参数量: 560.901K
    • FLOPs: 40.634G
    2025-05-04 16:20:50,850  - CRITICAL - === [FINAL TEST METRIC] ===                          
    ╒═══════════════╤════════╤════════╤═══════╤═══════╕
    │ Metric_Name   │   MEAN │     ET │    TC │    WT │
    ╞═══════════════╪════════╪════════╪═══════╪═══════╡
    │ Dice          │  0.844 │  0.797 │ 0.842 │ 0.894 │
    ├───────────────┼────────┼────────┼───────┼───────┤
    │ Jaccard       │  0.773 │  0.719 │ 0.777 │ 0.823 │
    ├───────────────┼────────┼────────┼───────┼───────┤
    │ Precision     │  0.869 │  0.817 │ 0.878 │ 0.913 │
    ├───────────────┼────────┼────────┼───────┼───────┤
    │ Recall        │  0.858 │  0.83  │ 0.848 │ 0.894 │
    ├───────────────┼────────┼────────┼───────┼───────┤
    │ H95           │  8.808 │ 11.594 │ 8.462 │ 6.369 │
    ╘═══════════════╧════════╧════════╧═══════╧═══════╛
    Mean Loss: 0.1568;ET: 0.2046;ET: 0.2046;TC: 0.1584;WT: 0.1074
    """
    def __init__(self, 
                 in_channels=4, 
                 out_channels=4,
...
#         cat_feats = torch.cat(branch_ouputs, dim=1)
#         weights = self.attn(self.gap(cat_feats)).view(-1, len(self.kernel), 1, 1, 1)
#         feats = cat_feats * weights
        
#         out = torch.sum(feats, dim=1, keepdim=True)
...
class DynamicCrossLevelAttention(nn.Module): #MSFA
    def __init__(self, 
        ...
        self.channel_attn = nn.Sequential(  #TODO: 尝试将每层的
            nn.Conv3d(
                ch_list[-1],
                ch_list[-1] // 4,
                kernel_size=1
    ...
    def forward(self, encoder_feats, x):
        squeezed_feats = []
        ca_weights = self.channel_attn(self.gap(x) + self.gmp(x))
        # 压缩通道数
        ...
        # 获取通道注意力
        downs = [down * ca_weights[:, i, :, :, :] for i, down in enumerate(downs)]
