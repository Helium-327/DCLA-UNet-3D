...
class SlimSepUNetv1(nn.Module):
    def __init__(self, 
        ...
        super(SlimSepUNetv1, self).__init__()
        
        self.MaxPool = nn.MaxPool3d(kernel_size=2, stride=2)
        self.Conv1 = SlimDownBlock(in_channels, f_list[0], kernel_size=kernel_size)
        self.Conv2 = SlimDownBlock(f_list[0], f_list[1], kernel_size=kernel_size)
        self.Conv3 = SlimDownBlock(f_list[1], f_list[2], kernel_size=kernel_size) 
        self.Conv4 = SlimDownBlock(f_list[2], f_list[3], kernel_size=kernel_size)
        ...
        self.UpConv5 = MultiKernelSlimUpBlock(f_list[3]*2, f_list[3]//2, kernel_size=3, dilations=[1, 2, 3])
        
        self.Up4 = UpSample(f_list[2], f_list[2], trilinear)
        self.Att4 = AttentionGate(F_g=f_list[2], F_l=f_list[2], F_inter=f_list[2]//2)
        self.UpConv4 = MultiKernelSlimUpBlock(f_list[2]*2, f_list[2]//2, kernel_size=3, dilations=[1, 2, 3])
        
        self.Up3 = UpSample(f_list[1], f_list[1], trilinear)
        self.Att3 = AttentionGate(F_g=f_list[1], F_l=f_list[1], F_inter=f_list[1]//2)
        self.UpConv3 = MultiKernelSlimUpBlock(f_list[1]*2, f_list[1]//2, kernel_size=3, dilations=[1, 2, 3])
        
        self.Up2 = UpSample(f_list[0], f_list[0], trilinear)
        self.Att2 = AttentionGate(F_g=f_list[0], F_l=f_list[0], F_inter=f_list[0]//2)
        self.UpConv2 = MultiKernelSlimUpBlock(f_list[0]*2, f_list[0], kernel_size=3, dilations=[1, 2, 3])
...
class SlimSepUNetv1_LKDCLA(SlimSepUNetv1):
    def __init__(self, 
                 in_channels=4, 
                 out_channels=4, 
                 kernel_size=7, 
        ...
        super(SlimSepUNetv1_LKDCLA, self).__init__(
...
if __name__ == "__main__":
    test_unet(model_class=SlimSepUNetv1_LKDCLA, batch_size=1)   