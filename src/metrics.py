...
def hausdorff_distance_3d(y_pred, y_mask, spacing=(1, 1, 1), percentile=95):
    ...
    for b in range(batch_size):
        ...
        mask = y_mask[b]
        # 空掩码处理逻辑重构
        mask_empty = np.sum(y_mask) == 0
        pred_empty = np.sum(y_pred) == 0
        
        if mask_empty ^ pred_empty:  # XOR运算
            max_distance = np.sqrt(np.sum((np.array(y_mask.shape[1:])*spacing)**2))
            return max_distance
        

        # 计算预测掩码的表面
        edge_pred = np.logical_xor(pred, binary_erosion(pred, iterations=1))
        edge_mask = np.logical_xor(mask, binary_erosion(mask, iterations=1))
        
        # 使用并行距离变换
        dt_mask = distance_transform_edt(~edge_mask, sampling=spacing)
        dt_pred = distance_transform_edt(~edge_pred, sampling=spacing)    
        
        # 计算真实掩码的表面
        dist_pred = dt_mask[edge_pred]
        dist_gt = dt_pred[edge_mask]
        # 合并距离
        surface_distances = np.concatenate([dist_pred, dist_gt])

         # 检查surface_distances是否为空
...
class EvaluationMetrics:
    def __init__(self, smooth=1e-5, num_classes=4, device=None):
        self.smooth = smooth
    ...
    def hausdorff_95(self, y_pred, y_mask):
        ...
        return np.array([mean_h95_distances, et_h95_distances, tc_h95_distances, wt_h95_distances])
        
    def update(self, y_pred, y_mask, hd95=False):
        """
        更新评估指标
        :param y_pred: 预测标签
        :param y_mask: 真实标签
        ...
        metrics = [
        torch.tensor(dice_scores, dtype=torch.float32, device=self.device),
        torch.tensor(jacc_scores, dtype=torch.float32, device=self.device),
        torch.tensor(precision_scores, dtype=torch.float32, device=self.device),
        torch.tensor(recall_scores, dtype=torch.float32, device=self.device),
        ]
        if hd95:
            h95_distance = self.hausdorff_95(y_pred, y_mask)
            metrics.append(torch.tensor(h95_distance, dtype=torch.float32, device=self.device))
        metrics = torch.stack(metrics, dim=0) # [7, 4]
        return metrics
